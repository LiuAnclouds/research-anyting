...
验证安装：
```bash
python3 -c "import sklearn, scipy, matplotlib, numpy; print('依赖安装成功')"
```

### 常见问题

**`pip: command not found`**：使用 `pip3` 或 `python3 -m pip` 代替。
**`python3: command not found`**（Windows）：从 [python.org](https://www.python.org/downloads/) 安装 Python（不要使用 Microsoft Store 版本），安装时勾选"Add Python to PATH"。
**`ModuleNotFoundError: No module named 'sklearn'`**：执行 `pip install scikit-learn`（注意：包名是 `scikit-learn`，导入名是 `sklearn`）。
**SSL 证书错误**：在受限网络环境中，设置环境变量 `MOON_INSECURE_SSL=1` 跳过 SSL 验证。
